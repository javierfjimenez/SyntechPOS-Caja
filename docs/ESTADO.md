...
- [ ] **🆕 Marcas (Brand) ahora se replican** — RESUELTO en SyntechPOS@80e5154 (D23): el delta de catálogo trae `brands: [{ id, name, is_active, row_version }]` y cada producto trae `brand_id` (nullable). La caja debe: (1) crear la tabla `brands` en su SQLite, (2) guardar `brand_id` en products al sincronizar, (3) exponer marca en búsqueda/filtro de la pantalla de venta. La marca es ATRIBUTO (no afecta precio ni fiscalidad). Tolerar su ausencia si el servidor es viejo. Spec: eventos-sync §7.1 + esquema (tabla brands)
- [ ] **🆕 GRID DE PRODUCTOS con avatar — DESBLOQUEADO** (SyntechPOS@c8d9170, D24). El servidor ya da todo lo necesario; construir el grid en la caja. **Contrato y reglas**:
