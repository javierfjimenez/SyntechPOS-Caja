...
- [ ] **🆕 TIPOGRAFÍA DE MARCA cambió** — DISENO actualizado en SyntechPOS@affbbd2: la familia pasó de Inter → **Manrope** (sans) y de JetBrains Mono → **IBM Plex Mono** (montos/códigos). El backoffice ya las usa. La caja debe alinear sus fuentes en `tailwind.config` / CSS para mantener la coherencia de marca (DISENO §4, ambos frontends comparten tokens). Cargarlas desde Bunny Fonts (`manrope`, `ibm-plex-mono`).
...
- **Pulido del rediseño (2026-06-11)**: menú en el avatar del topbar (Transacciones del turno, Estado, Impresora, Calculadora, Pantalla completa, Cambiar cajero) que restaura los accesos quitados de la toolbar nueva; **todos los modales de venta al chrome cobalt** `ModalPro` (Cliente, Efectivo, En espera, Producto desconocido, Anular, Transacciones recientes); F11 pantalla completa de vuelta. Login/Apertura/Vinculación/Estado/Devolución heredan los tokens cobalt (cero hex hardcodeados). Pendiente menor: Calculadora/ConfiguracionImpresora siguen en ModalBase (funcionan, tokens cobalt); Devolución sigue como ruta (funciona, no modal)
